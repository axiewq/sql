...
SELECT shipped_date - order_date as days_to_ship FROM orders WHERE ship_country = 'Germany'
...
SELECT MAX(unit_price), MIN(unit_price) FROM products WHERE discontinued IS NOT NULL
...
SELECT MAX(unit_price), MIN(unit_price) FROM products WHERE discontinued IS NOT NULL AND units_in_stock <= 20
